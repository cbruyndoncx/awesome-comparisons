import { deleteUndefinedKeys, getDefaultCriteria, isNullOrUndefined, resolveDefault } from "../util.js";
import { CriteriaValue } from "./criteriaValue.js";

const defaultConfiguration = require("../default");

class Criteria {
  constructor(id, type, name, search, table, detail, description, placeholder, order, andSearch, rangeSearch, values, lock, children, defaultExpanded, useDefaults = true) {
    const ref = {
      id: id,
      type: type,
      name: name,
    ...
    if (isNullOrUndefined(ref.defaultExpanded)) {
      ref.defaultExpanded = false;
    }
    const hasGroupingChildren = Array.isArray(ref.children) && ref.children.length > 0;
    ref.defaultExpanded = hasGroupingChildren && ref.defaultExpanded === true;
    ...
    this.defaultExpanded = hasChildren && ref.defaultExpanded === true;
  }
  ...
  static load(key, type, json, defaultConfig, useDefaults = true) {
    ...
    if (isNullOrUndefined(json)) {
      ...
      const rawExpanded = !isNullOrUndefined(json.defaultExpanded) ? json.defaultExpanded : json.expanded;
      const defaultExpanded = hasChildren && rawExpanded === true;
      return new Criteria(
        criteriaKey,
        type,
        json.name,
  ...
  static loadArray(json, defaultConfig, useDefaults = true) {
    ...
    if (useDefaults) {
      ...
      if (!criteriaNames.has("ShortDescription")) {
        const criteriaSource = !isNullOrUndefined(defaultConfig) && Array.isArray(defaultConfig.criteria)
          ? defaultConfig.criteria
          : [];
        const defaultShort = criteriaSource.find(criteria => criteria.hasOwnProperty("ShortDescription"));
        const shortConfig = defaultShort ? defaultShort.ShortDescription : null;
        const shortType = shortConfig && shortConfig.type ? shortConfig.type : CriteriaTypes.MARKDOWN;
        result.push(Criteria.load("ShortDescription", shortType, shortConfig, defaultConfig, useDefaults));
      }
  ...
  static empty(key, type, useDefaults = true, defaultConfiguration) {
    ...
    return new Criteria(key, type, null, null, null, null, null, null, null, null, null, null, null, null, null, useDefaults);
  }
  ...
  json() {
    ...
    criteria[this.id] = deleteUndefinedKeys({
      ...
      defaultExpanded: this.defaultExpanded === true ? true : undefined
    });
    return criteria;
  }
}
