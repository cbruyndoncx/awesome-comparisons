...
class Label {
  constructor(name, tooltip, clazz, color, backgroundColor, stars, comment, display, displayHtml) {
    this.name = name;
  ...
  static loadJson(json, criteria) {
    ...
    if (!isNullOrUndefined(criteria)) {
      ...
      if (!isNullOrUndefined(criteriaValue)) {
        ...
        display = criteriaValue.display;
        displayHtml = criteriaValue.displayHtml;
      }
    ...
    return new Label(name, tooltip, clazz, color, backgroundColor, stars, comment, display, displayHtml);
  }
