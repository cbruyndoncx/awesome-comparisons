...
import { Configuration, Criteria, CriteriaTypes, Data, isNullOrUndefined, Label, Tooltip } from "../model/model.module.js";
...
export function developmentColumn(files, done) {
    ...
    const autoConfiguration = Configuration.load(yaml2json.load(readFileSync(files.autoConfig, "utf8")));
    ...
    data.dataElements.forEach(dataElement => {
        if (dataElement.criteriaData.has(criteria.id)) {
            ...
            if (link.toLowerCase().indexOf('github.com') === -1) {
                // No repository enrichment possible for non GitHub links; clear any auto labels.
                criteriaData.labels = new Map();
                return;
            }
...
function getCommit(link) {
    ...
    if (link.indexOf('github.com') > -1) {
        ...
        downloadMap[link] = fetch(query, { headers })
            .then(response => {
                if (!response.ok) {
                    return Promise.reject(new Error(`GitHub API ${response.status}: ${response.statusText} (${query})`));
                }
