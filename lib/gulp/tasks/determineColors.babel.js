import { Criteria, CriteriaValue, isNullOrUndefined } from "../model/model.module.js";
...
export function determineColors(defaultConfiguration, configuration, oldAutoConfiguration, autoConfiguration, done) {
    ...
    if (!isNullOrUndefined(colorCriteria) && colorCriteria.values.size > 0) {
        ...
        // Guard: when criteriaValueCount or criteriaCount is 0, delta becomes Infinity,
        // and Infinity % length is NaN, which makes colorValues[NaN] undefined and crashes.
        if (!Number.isFinite(delta) || !Number.isFinite(columnDelta)) {
            return done();
        }
