...
export function criteria(paths, files, done) {
    ...
    data.dataElements.forEach(dataElement => {
        (dataElement.criteriaData || new Map()).forEach(criteriaData => {
            ...
            // Normalize the criteriaData name to its canonical id so the rewritten
            // data.json content matches the criterion key the UI looks up by.
            // Without this, headers like "Insurance Provider" never match column id "InsuranceProvider".
            criteriaData.name = criteriaId;
    ...
    (data.groups || new Map()).forEach((summary, groupKey) => {
        ...
        if (!isNullOrUndefined(summary.children)) {
            // Normalize child references to canonical criterion ids so the UI
            // can match group children against the actual column ids.
            targetCriteria.children = summary.children.map(childName =>
                resolveCriteriaId(criteriaAliasMap, childName)
            );
