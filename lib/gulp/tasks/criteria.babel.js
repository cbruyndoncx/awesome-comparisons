...
function processMcpEnrichment(dataElement, configuration) {
    if (!dataElement.mcpEnrichment) {
        // No enrichment data yet (mcp:enrich runs after this task) — preserve markdown values
        return;
    }
    ...
    // Set MCP-Client to "Yes" — use criteria.id as map key to avoid duplicates
    if (configuration.containsCriteria('MCP-Client')) {
        ...
        dataElement.criteriaData.set(criteria.id, labelData);
    }
    ...
    if (configuration.containsCriteria('MCP-Protocol-Version') && enrichment.protocolVersion) {
        ...
        dataElement.criteriaData.set(criteria.id, textData);
    }
