...
import { registerMcpEnrichmentTasks } from "./tasks/mcp-enrichment.babel.js";
...
// Register MCP enrichment tasks
registerMcpEnrichmentTasks(gulp, paths, datasetContexts);
...
function assets() {
    ...
    const imagePromises = datasetContexts.map(dataset => {
        ...
        return copyDatasetImages(dataset);
    });
    copyManifestToAngularAssets();
    return Promise.all(imagePromises);
}

async function copyDatasetImages(dataset) {
    const dataDirs = dataset.sources.dataDirs || [dataset.sources.dataDir];
    for (const dataDir of dataDirs.filter(Boolean)) {
        const sourceImagesDir = path.join(dataDir, 'images');
        if (!existsSync(sourceImagesDir) || !statSync(sourceImagesDir).isDirectory()) {
            continue;
        }
        await ensureDatasetThumbnails(sourceImagesDir);
        const distImagesTarget = path.join(dataset.distDir, 'images');
        ensureDirExists(distImagesTarget);
        copyDirectoryContents(sourceImagesDir, distImagesTarget);
        assetTargets.forEach(({ target, createIfMissing }) => {
            if (!target) {
                return;
            }
            const destination = path.join(target, dataset.assetRelativePath, 'images');
            ...
            try {
                copyDirectoryContents(sourceImagesDir, destination);
            } catch (error) {
                console.warn(`Unable to copy images for dataset ${dataset.id} to ${destination}: ${error.message}`);
            }
        });
    }
}

async function ensureDatasetThumbnails(sourceImagesDir) {
    const thumbsDir = path.join(sourceImagesDir, 'thumbs');
    ensureDirExists(thumbsDir);
    const imageExt = /\.(png|jpe?g|webp)$/i;
    const tasks = readdirSync(sourceImagesDir, { withFileTypes: true })
        .filter(entry => entry.isFile() && imageExt.test(entry.name))
        .map(entry => {
            const source = path.join(sourceImagesDir, entry.name);
            const target = path.join(thumbsDir, entry.name);
    ...
    await Promise.all(tasks.map(({ source, target }) =>
        sharp(source)
            .resize({ width: 240, height: 160, fit: 'cover' })
            .toFile(target)
            .catch(err => console.warn(`Thumbnail failed for ${source}: ${err.message}`))
    ));
}
...
// MCP criteria processing task (runs after mcp:enrich) -------------<
gulp.task('mcp-criteria', function (done) {
    datasetContexts.forEach(context => {
        const files = buildContextFiles(paths, context);
        const { Configuration, Data } = require('./model/model.module.js');
        const { readFileSync, writeFileSync, existsSync } = require('fs');
        const yaml2json = require('js-yaml');

        if (!existsSync(files.dataJson)) {
            return;
        }

        // Load configuration
        const configuration = Configuration.load(yaml2json.load(readFileSync(files.config, "utf8")));
        const autoConfiguration = Configuration.load(
            existsSync(files.autoConfig) ? yaml2json.load(readFileSync(files.autoConfig, "utf8")) : {}
        );
        configuration.combine(autoConfiguration);

        // Load data (now with mcpEnrichment from mcp:enrich task)
        const data = Data.loadJson(JSON.parse(readFileSync(files.dataJson, "utf8")), configuration);

        // Import MCP processing functions
        const { processMcpEnrichment } = require('./tasks/criteria.babel.js');
