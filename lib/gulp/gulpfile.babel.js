...
function copyToAssets(sourcePath, fileName) {
    assetTargets.forEach(({ target, createIfMissing }) => {
        ...
        try {
            copyFileSync(sourcePath, path.join(target, fileName));
        } catch (error) {
            console.warn(`Unable to copy ${fileName} to assets (${target}): ${error.message}`);
        }
    });
}
...
function markdown(callback) {
    ...
    const command = `node "${cliPath}" --input "${paths.data}" --output "${files.dataJson}" --tmp "${paths.json}"`;
    console.log(command);
...
gulp.task('development-column', function (done) {
    developmentColumn(files, function (err) {
        if (!err) {
