...
```bash
# Watch mode (data + config workspace + Angular dev server)
npm run dev -- --dataset <dataset-id>

# One-time data prepare + serve
npm start -- --dataset <dataset-id>
```

What this does:
- Runs gulp data pipeline (md → JSON) for selected dataset(s)
- Starts the config workspace backend at http://localhost:3100
- Serves the Angular app at http://localhost:4200 with a proxy for `/api/config`
- Watches for changes in data/config (dev mode)

Open http://localhost:4200 (main app) and http://localhost:4200/admin/config (config editor)
...
npm run data:prepare -- --dataset <dataset-id>

# Watch data/config/style and rebuild on changes
npm run data:watch -- --dataset <dataset-id>
...
npm run build:prod
# or a targeted build
npm run build -- --dataset <dataset-id> --clean

# Stamp version info and prepare a release
npm run release
```

---

### Step 1.5: Gulp/Data Tasks

Use the npm wrappers for gulp tasks (they compile and run the TypeScript md2json CLI under `lib/md2json/dist/cli.js`):
...
RouterOutlet
    ...
    │     └→ ComparisonComponent (rendered inside shell)
    └→ ConfigAdminShellComponent (/admin/config)
```

**File Locations:**
- `src/app/app.component.ts` - Root component
- `src/app/app.module.ts` - Routing configuration (RouterModule.forRoot)
...
// Generated by scripts/generate-environment.js from package.json
export const environment = {
  production: true,
  repositoryUrl: 'https://github.com/<owner>/<repo>',
  githubPagesUrl: 'https://<owner>.github.io/<repo>'
...
Notes:
- The config workspace API uses a relative base `/api/config` and is proxied to `http://localhost:3100` in development (`proxy.config.json`).
- `environment.prod.ts` is auto-generated during build by `npm run env:generate` (invoked by `build:prod`).
